...
async function tryWriteNodes(session, res, nodesToWrite, operationName) {
    try {
        ...
        if (statusCodes[0].isGood()) {
            ...
            return res.status(500).json({ error: `${operationName} failed`, statusCode: statusCodes[0].toString() });
        }
    } catch (error) {
...
async function moveRight(res) {
    ...
    const response = await tryWriteNodes(session, res, nodesToWrite, "Move Right")
...
async function closeClaw(res) {
    ...
    const response = await tryWriteNodes(session, res, nodesToWrite, "Close Claw")
    await closeSession(session);
    await disconnectFromOPCUAServer(client);
    return response;
}